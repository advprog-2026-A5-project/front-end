...
function MandorPengiriman({ actor }: { actor: { id: number; role: Role } }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    const results = await Promise.allSettled([loadDrivers(), loadHarvests(), loadOngoing()]);
    const failed = results.find((result) => result.status === "rejected");

    if (failed && failed.status === "rejected") {
      setError(failed.reason instanceof Error ? failed.reason.message : "Sebagian data pengiriman gagal dimuat");
    }
        ...
        <form className="mt-3 space-y-3" onSubmit={assign}>
          <label className="block text-sm text-slate-300">
            ...
            <div className="mt-1 grid grid-cols-[1fr_auto] gap-2">
              ...
              />
              <button
                className="rounded-md border border-slate-500 px-3 py-2 text-sm text-slate-100 hover:bg-slate-800"
                type="button"
                onClick={() => void runAction(loadDrivers, "Gagal mencari supir")}
              >
                Cari
              </button>
            </div>
          ...
          >
            ...
            {drivers.map((driver) => (
              <option key={driver.id} value={driver.id}>
                {driver.nama ?? driver.username ?? `Supir #${driver.id}`}
              </option>
            ))}
          </select>
          ...
          <div className="rounded-md border border-slate-700 p-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                type="button"
                onClick={() => void runAction(loadHarvests, "Gagal memuat panen approved")}
              >
                Refresh
              </button>
            </div>
            ...
            <div className="mt-2 max-h-64 space-y-2 overflow-auto">
              {harvests.map((harvest) => {
                ...
                return (
                  <label
                    ...
                    className="flex gap-2 rounded-md border border-slate-700 bg-slate-900/70 p-2 text-sm text-slate-200"
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="text-xs text-slate-400">
                        {harvest.buruhName} | {formatKg(detail?.kilogram)}
                      </span>
                    </span>
                  </label>
                );
              ...
              {harvests.length === 0 && <p className="text-sm text-slate-400">Belum ada panen approved.</p>}
            </div>
          </div>
          ...
          <button
            ...
            type="submit"
            disabled={!selectedDriverId || selectedHarvestIds.length === 0 || selectedWeight <= 0 || selectedWeight > 400}
          >
            Assign Supir
          </button>
        </form>
      ...
      <section className="rounded-lg border border-slate-700 bg-slate-950/50 p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button
            className="rounded-md border border-slate-500 px-3 py-2 text-sm text-slate-100 hover:bg-slate-800"
            type="button"
            onClick={() => void runAction(loadOngoing, "Gagal memuat pengiriman berlangsung")}
          >
            Refresh
          </button>
        </div>
          ...
          action={(shipment) =>
            shipment.status === "TIBA_DI_TUJUAN" ? (
              <div className="flex gap-2">
                <button className="rounded bg-emerald-700 px-2 py-1 text-white hover:bg-emerald-600" onClick={() => void processMandor(shipment.id, "approve")}>
                  Approve
                </button>
                <button className="rounded bg-red-700 px-2 py-1 text-white hover:bg-red-600" onClick={() => void processMandor(shipment.id, "reject")}>
                  Reject
                </button>
              </div>
            ) : (
...
function SupirPengiriman({ actor }: { actor: { id: number; role: Role } }) {
  ...
  const loadHistory = useCallback(async () => {
    setHistory(await pengirimanApi.driverHistory(actor.id, actor.role, actor.id, {
  ...
  const loadAll = useCallback(async () => {
    const results = await Promise.allSettled([loadActive(), loadHistory()]);
    const failed = results.find((result) => result.status === "rejected");

    if (failed && failed.status === "rejected") {
      setError(failed.reason instanceof Error ? failed.reason.message : "Gagal memuat data supir");
    }
      ...
      <section className="rounded-lg border border-slate-700 bg-slate-950/50 p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button
            className="rounded-md border border-slate-500 px-3 py-2 text-sm text-slate-100 hover:bg-slate-800"
            type="button"
            onClick={() => void runAction(loadActive, "Gagal memuat tugas aktif")}
          >
            Refresh
          </button>
        </div>
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              className="rounded-md border border-slate-500 px-3 py-2 text-sm text-slate-100 hover:bg-slate-800"
              type="button"
              onClick={() => void runAction(loadHistory, "Gagal memuat riwayat pengiriman")}
            >
              Filter
            </button>
          </div>
...
function AdminPengiriman({ actor }: { actor: { id: number; role: Role } }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setShipments(await pengirimanApi.approvedForAdmin(actor.id, actor.role, {
        mandorName: mandorName || undefined,
        date: date || undefined,
      }));
    } catch (e) {
      ...
      <ShipmentTable
        ...
        action={(shipment) => (
          <div className="flex flex-wrap gap-2">
            <button className="rounded bg-emerald-700 px-2 py-1 text-white hover:bg-emerald-600" onClick={() => void processAdmin(shipment, "approve")}>
              Approve
            </button>
            <button className="rounded bg-amber-700 px-2 py-1 text-white hover:bg-amber-600" onClick={() => void processAdmin(shipment, "partial")}>
              Partial
            </button>
            <button className="rounded bg-red-700 px-2 py-1 text-white hover:bg-red-600" onClick={() => void processAdmin(shipment, "reject")}>
              Reject
            </button>
          </div>
        )}
    ...
    <div className="overflow-auto rounded-lg border border-slate-700">
      <table className="min-w-full text-sm">
        ...
        <tbody>
          ...
          {shipments.length === 0 && (
            <tr>
              <td className="p-4 text-center text-slate-400" colSpan={action ? 8 : 7}>
                Tidak ada data.
              </td>
            </tr>
          )}
